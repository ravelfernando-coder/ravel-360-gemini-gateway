...
begin
  ...
  if p_operation = 'health' then
    ...
    if owner_row.status <> 'active' then
      return jsonb_build_object('ok', false, 'code', 'OWNER_NOT_ACTIVE');
    end if;
    ...
    update public.gateway_requests
       set state = p_payload->>'state',
           response = case
             when p_payload ? 'response' then p_payload->'response'
             else response
           end,
           error = case
    ...
    if found then
      ...
      then
        return jsonb_build_object('ok', false, 'code', 'IDEMPOTENCY_IN_PROGRESS');
      end if;
